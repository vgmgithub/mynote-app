...
export const PRIVACY = [
  ...
  ['Usage information', [
    ...
    'Your anonymous name (for example Meharika) is a made-up one-word name given to this install so you can ask us for help without telling us who you are. It is drawn at random, never from anything you typed, and it is sent with the counts below so we can find your install if you quote it. If you gave a gender it is used once, when the name is chosen; the name then stays the same for good, so changing or removing your gender later does not rename you and a name you gave us weeks ago still works.',
